...
import cn from 'clsx'
import { MarketSteps, MarketTransactionType, useMarketManager } from "@/stores";
  ...
  useEffect(() => {
    if (isTxError) {
      const errorMessage =
        ...
        toast.fail({ message: errorMessage });
    }
  }, [isTxError]);
        ...
        >
          ...
          <div className="flex flex-col space-y-2">
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    console.log(error);
                    toast.fail({ message: 'Error switching chain.' });
                  }
                }}
              >
            ...
            {(!allTransactionsExecuted || isTransactionTimeout) && (
              <DialogClose asChild>
                <Button
                  className="h-9 bg-[#eb3c27] text-sm"
                  onClick={handleClose}
                  type="button"
                  disabled={
                    isTxPending || isTxConfirming || isTransactionTimeout
          ...
          <TransactionConfirmationModal
            ...
            onConfirm={() => {
              try {
                ...
                setIsTransactionTimeout(false);
                toast.fail({ message: 'Error submitting transaction' });
              }
            }}
          />
        </DialogContent>
