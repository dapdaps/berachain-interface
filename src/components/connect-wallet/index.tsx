...
const ConnectWallet = ({ className }: { className?: string }) => {
  ...
  const [mobileUserInfoVisible, setMobileUserInfoVisible] = useState<boolean>(false);
  const [mobileNetworksVisible, setMobileNetworksVisible] = useState<boolean>(false);
  ...
  return (
    <>
      {
        isConnecting ? (
          <Skeleton width={125} height={42} borderRadius={21} style={{ transform: 'translateY(-4px)' }} />
        ) : (
          isConnected ? (
            <div className='flex justify-start items-center gap-x-[20px] pl-2 pr-3'>
              <motion.div
                ...
                >
                  {
                    address && userInfo?.avatar ? (
                      <img src={userInfo?.avatar} alt="" className="w-[28px] h-[28px] rounded-full" />
                    ) : (
