...
import {
  ...
  balanceToDecimal,
} from "./service/balanceTo";
import { getBalanceNearAllowedToSwap } from "./service/getBalanceNearAllowedToSwap";
  ...
  const handleEstimateSwap = async ({
    ...
    try {
      ...
      const { bestEstimate } = await debouncedGetSwapEstimateBot({
        ...
        amountIn: utils
          .parseUnits(tokenIn, selectTokenIn?.decimals ?? 0)
          .toString(),
      });
