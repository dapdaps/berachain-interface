...
export default function Input({ value, maxTicket, onChange }: any) {
  return (
    <div className="border border-[#373A53] bg-white rounded-[12px] p-[10px] flex items-center">
      <button
        className={clsx(
          value === 1 ? "opacity-50 cursor-not-allowed" : "cursor-pointer"
        )}
        onChange={() => {
          if (value === 1) return;
          onChange(value - 1);
      ...
      <div className="text-center text-[16px] text-black font-semibold grow">
        {value}
      </div>
      <button
        className={clsx(
