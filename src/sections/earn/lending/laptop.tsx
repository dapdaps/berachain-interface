...
const getListMeta = (tabType: 'Supply' | 'Borrow', { handleSwap, handleAction, addAction, honeyInfo }: any) => {
  ...
  const supplyColumns = [
    ...
    },
    {
      ...
      sort: true,
      width: "15%",
      render: (item: any) => numberFormatter(item.inWallet, 2, true, { prefix: '$' }),
    },
    {
      title: "You Supplied",
      key: "youSupplied",
  ...
  const borrowColumns = [
    ...
    },
    {
      ...
      sort: true,
      width: "15%",
      render: (item: any) => numberFormatter(item.borrowCapacity, 2, true, { prefix: '$' }),
    },
    {
      title: "You Borrowed",
      key: "youBorrowed",
      ...
      render: (item: any) => (
        <div className="flex items-center gap-1">
          <img src={item.icon} className="w-[20px] h-[20px]" alt="" />
          <div className="underline">{numberFormatter(item.youBorrowed, 2, true)}</div>
        </div>
      ),
    },
    {
  ...
  return [...commonColumns, ...(tabType === 'Supply' ? supplyColumns : borrowColumns)];
};
