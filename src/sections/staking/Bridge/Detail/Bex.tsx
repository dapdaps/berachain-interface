...
function DolomiteButton({ id, onSuccess }: { id: string }) {
  ...
  const token = useMemo(
    () =>
      supplyTokens?.find((supplyToken) => supplyToken?.symbol === id) ?? null,
    supplyTokens
  );
  ...
  return (
    <>
      {token && (
        ...
        >
          <div className="cursor-pointer flex items-center justify-center w-[148px] h-[46px] rounded-[10px] border border-black bg-[#FFDC50]">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-[10px]">
            ...
            <div className="flex flex-col gap-[4px]">
              ...
              <IbgtAmount
                className="text-black font-Montserrat text-[16px] font-semibold leading-[100%]"
                symbol={data?.id}
                usdAmount={data?.usdDepositAmount}
