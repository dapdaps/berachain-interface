...
export default function Invest() {
    ...
    const filterList = useMemo(() => {
        const _filterList = dataList
            ...
            .filter(data => rateKey === "Single" ? data?.tokens?.length === 1 : data?.tokens?.length === 2)
        return sortDataIndex ? _.cloneDeep(_filterList).sort((prev, next) => Big(next[sortDataIndex]).minus(prev[sortDataIndex]).toFixed()) : _filterList
