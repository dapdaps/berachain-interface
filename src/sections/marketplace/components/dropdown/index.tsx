...
export default function Dropdown({ list, value, onChange, placeholder }: Props) {
    ...
    return <div ref={domRef} onClick={() => {
        ...
        <div>
            <svg width="14" height="8" viewBox="0 0 14 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13 1L7 5.8L1 0.999999" stroke="black" stroke-width="2" stroke-linecap="round" />
            </svg>

        </div>
        {
            modalShow && <div className="absolute w-[100%]  overflow-auto bg-[#fff] rounded-[8px] border border-[#373A53] right-0 top-[45px]">
                {
                    list?.map(item => {
                        return <div key={item.key} onClick={(e) => {
                            e.stopPropagation()
                            onChange(item.key)
                            setModalShow(false)
                            ...
                            <div>{item.name}</div>
                            {
                                item.key === value && <div className="w-[6px] h-[6px] bg-[#EBF479] absolute top-[18px] right-1 rounded-[6px]"></div>
