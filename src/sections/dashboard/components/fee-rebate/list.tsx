...
const FeeRebateList = ({ className }: FeeRebateListProps) => {
  ...
  const [hasSelected] = useMemo(() => {
    return [transactions.some((it: any) => it.selected && !it.rebated)];
  }, [transactions]);
  ...
  const getList = () => {
    ...
    const _transactions = _list.map((record: any, idx: number) => {
      return {
        key: record.id,
        ...record,
        id: record.id,
  ...
  const handleSelected = (record: any) => {
    if (record.rebated || isPending) return;
  ...
  return (
    <div className={clsx('flex flex-col gap-2', className)}>
                    ...
                    >
                      <div
                        className={clsx(
                          ...
                          (record.rebated || isPending) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'
                        )}
      ...
      <button
        ...
        disabled={isPending || isLoading || !hasSelected}
      >
