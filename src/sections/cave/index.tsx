...
export default function Cave() {
    ...
    return <div className=" relative w-[100vw] h-[100vh]">
        ...
        <div className="flex items-end px-[30px] absolute w-[583px] left-[50%] top-[270px] translate-x-[-50%] ">
            {
                hats.map(item => {
                    return <div className="flex-1">
                        <img style={hatPositions[item.level - 1]} src={`/images/cave/hat/hat-${item.level}${item.pc_item ? '-' + item.level : ''}.png`} />
                    </div>
                })
            }
            ...
            <div className="flex items-end px-[30px] pt-[120px]">
                {
                    clothes.map(item => {
                        return <div className="flex-1">
                            <img className="w-[102px]" style={clothPositions[item.level - 1]} src={`/images/cave/clothing/cloth-${item.level}${item.pc_item ? '-' + item.level  : ''}.png`} />
                        </div>
                    })
                }
        ...
        <div style={{ left: 'calc(50% - 170px - 290px)' }} className="w-[349px] h-[230px] translate-x-[-50%] absolute top-[500px] bg-[url('/images/cave/box.png')] bg-contain bg-no-repeat bg-bottom">
            ...
            {
                cars.map(item => {
                    return <div className="absolute" style={carPositions[item.level - 1]}>
                        <img className="h-[78px]" style={item.pc_item ? carsSize[item.level - 1] : {}} src={`/images/cave/key/key-${item.level}${item.pc_item ? '-' + item.level : ''}.png`} />
                    </div>
                })
            }
        </div>
