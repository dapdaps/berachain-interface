...
const ActionUnionRight = (props: any) => {
  ...
  const { actionType, toggleActionType, currentProtocol } = useVaultsV2Context();

  return (
    <div className={clsx("w-full bg-[rgba(0,0,0,0.06)] rounded-[10px] p-[24px_20px_20px_20px]", className)}>
      <div className="relative grid grid-cols-2 h-[56px] flex-shrink-0 w-full rounded-[12px] border border-[#373A53] bg-white p-[5px_4px] text-center text-[18px] font-[600] leading-[90%] font-Montserrat text-[#000]">
        ...
        <motion.div
          ...
          animate={{
            x: actionType.value === ActionTypes[ACTION_TYPE.DEPOSIT].value ? 0 : '100%',
          }}
        />
      </div>
      {
        currentProtocol?.protocol === "BurrBear" ? (
          <BurrBear />
        ) : (
          (currentProtocol?.protocol === "Kodiak" && actionType.value === ACTION_TYPE.WITHDRAW) ? (
            <KodiakUnstake />
          ) : (
