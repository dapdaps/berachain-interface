...
const ActionUnionPotions = (props: any) => {
  ...
  return (
    <div className={clsx("w-full pl-[17px] pr-[2px] border-b border-[rgba(0,0,0,0.15)] pb-[46px]", className)}>
          ...
          <div className="flex items-center">
            {
              currentProtocol?.tokens?.map((token: any, index: number) => (
                <LazyImage
                  key={index}
                  src={token.icon}
                  containerClassName={clsx("!w-[30px] !h-[30px] rounded-[50%] overflow-hidden shrink-0", index > 0 && "ml-[-10px]")}
