...
const Navigation = (props: any) => {
  ...
  const handleDragStart = (event: MouseEvent, info: any) => {
    ...
    navigationStartPointPositionRef.current = {
      x: info.point.x,
      y: info.point.y
    };
  ...
  const handleDragEnd = (event: MouseEvent, info: any) => {
    setIsDragging(false);
    navigationDragEndedTimesRef.current =
      navigationDragEndedTimesRef.current + 1;
  ...
  const handleDrag = (event: MouseEvent, info: any) => {
    ...
    const navigationRotate =
      navigationStartRotationRef.current +
      (info.point.x - navigationStartPointPositionRef.current.x) * sensitivity;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        width: size,
        height: size,
        backgroundColor: isRainyDay
          ? "#90AF4E"
          : themeConfig.earthBackgroundColor
      }}
      drag
      dragElastic={0}
      dragMomentum={false}
        ...
        entries
          ...
          .map((item: any, i: number) => (
            ...
            >
              <motion.img
                className={clsx(
                  "absolute left-1/2 top-[-80px] z-[1] object-center object-contain",
                  item.disabled ? "cursor-not-allowed" : "cursor-pointer",
                  isDefaultTheme() ? "w-[220px]" : "w-[300px]"
                )}
                src={
