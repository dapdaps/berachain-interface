...
export default memo(function gauge() {
  ...
  const getBalance = async (stakingTokenAddress, vaultAddress) => {
    const contract = new ethers.Contract(currentTab === "deposit" ? stakingTokenAddress : vaultAddress, ERC20_ABI, provider)
        ...
        <div className="mb-[30px] flex items-center justify-between pl-[19px] pr-[23px] h-[75px] rounded-[20px] bg-[#FFDC50]">
          <div className="flex items-center">
            ...
            <div className="ml-[32px] mr-[14px] w-[42px] h-[42px] rounded-full overflow-hidden">
              <img src={gaugeData?.metadata?.logoURI} alt={gaugeData?.metadata?.name} />
            </div>
            <div className="flex flex-col gap-[5px]">
              ...
              <div className="flex items-center gap-[4px]">
                ...
                <div className='w-[16px] h-[16px] rounded-full overflow-hidden'>
                  <img src={gaugeData?.metadata?.productMetadata?.logoURI} alt={gaugeData?.metadata?.product} />
                </div>
          ...
          <div className="flex flex-col gap-[12px]">
            <div className="text-black font-Montserrat text-[12px] font-medium">Staking Token : <span className="underline">0xa5...4B76</span></div>
            <div className="text-black font-Montserrat text-[12px] font-medium">Reward Vault : <span className="underline">0xa5...4B76</span></div>
          </div>
        </div>
                ...
                <div className="flex flex-col">
                  ...
                  <div className="mb-[14px] flex flex-col h-[72px] rounded-[12px] border border-[#373A53] bg-white">
                    ...
                    <div className="flex justify-end pr-[14px]">
                      <span className="text-[#3D405A] font-Montserrat text-[12px] font-medium">balance: {formatValueDecimal(state?.balance, '', 2)}</span>
                    </div>
                  </div>
                  <Button
                    type={currentTab}
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex flex-col h-[72px] rounded-[12px] border border-[#373A53] bg-white">
                    <div className="pt-[16px] pl-[20px] pr-[14px] flex items-center justify-between">
                      ...
                      <span className="text-black font-Montserrat text-[16px] font-semibold leading-[90%]">HONEY-PAW</span>
                    </div>
                    <div className="flex justify-end pr-[14px]">
                      <span className="text-[#3D405A] font-Montserrat text-[12px] font-medium">balance: {formatValueDecimal(state?.balance, '', 2)}</span>
                    </div>
            ...
            <div className="p-[20px] w-full h-[103px] rounded-[10px] bg-black/[0.06]">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-[11px]">
                  <span className="text-black font-Montserrat text-[16px] font-semibold leading-[90%]">{formatValueDecimal(state?.depositAmount, '', 2)}</span>
                  <span className="text-[#3D405A] font-Montserrat text-[12px] font-medium">{formatValueDecimal(Big(state?.depositAmount ? state?.depositAmount : 0).div(state?.totalSupply ? state?.totalSupply : 1).times(100).toFixed(), '', 2)}%</span>
                </div>
              </div>
            </div>

            <div className="py-[20px] px-[18px] w-full h-[184px] rounded-[10px] bg-black/[0.06]">
              ...
              <div className="mt-[30px] mb-[18px] flex items-center justify-between">
                ...
                <div className="flex items-center gap-[11px]">
                  <span className="text-black font-Montserrat text-[16px] font-semibold leading-[90%]">{formatValueDecimal(state?.earned, '', 2)}</span>
                  <span className="text-[#3D405A] font-Montserrat text-[12px] font-medium">{formatValueDecimal(Big(state?.earned ? state?.earned : 0).times(prices?.["BGT"] ?? 0).div(prices?.["USDC"] ?? 1).toFixed(), '$', 2)}</span>
                </div>
              </div>
              {
                ...
                <>
                  {
                    state?.claimLoading ? (
                      <div className="cursor-not-allowed opacity-50 flex items-center justify-center h-[60px] rounded-[10px] border border-black bg-[#FFDC50]">
                        <CircleLoading size={14} />
                      </div>
                    ) : (
                      <div className="cursor-pointer flex items-center justify-center h-[60px] rounded-[10px] border border-black bg-[#FFDC50]" onClick={handleClaim}>Claim</div>
                    )
                  }
                </>
              }
