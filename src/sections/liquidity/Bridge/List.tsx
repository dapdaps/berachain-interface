...
export default function List(props: any) {
  ...
  const columnList: ColunmListType = [
      ...
      render: (data) => {
        ...
        return (
          <div className='flex items-center gap-[8px]'>
            ...
            <div className='text-black font-Montserrat text-[16px] font-medium leading-[100%]'>{pool?.name || 'iBGT'}</div>
          </div>
        );
      }
    },
    {
      ...
      render: (data: any) => {
        ...
        return (
          <img
            style={{ width: 26 }}
            src={`/images/dapps/infrared/${pool ? (pool?.protocol === 'BEX' ? 'bex' : 'berps') : 'infrared'}.svg`}
          />
        );
      }
    },
    ...
    },
    {
      ...
      sort: true,
      render: (data) => {
        return <div className='text-black font-Montserrat text-[16px] font-medium leading-[100%]'>{Big(data?.apy ?? 0).toFixed(2)}%</div>;
      }
    },
    {
      width: '15%',
      ...
      sort: true,
      render: (data) => {
        return <div className={clsx('text-black font-Montserrat text-[16px] font-medium leading-[100%]', { 'opacity-30	': Big(data?.usdDepositAmount ?? 0).eq(0) })}>{formatValueDecimal(data?.usdDepositAmount, '$', 2, true, false)}</div>;
      }
    },
    {
      width: '15%',
  ...
  return (
    <div>
      <div className="pl-[18px] text-black font-Montserrat text-[26px] font-bold leading-[90%]">Vaults</div>
      <div className="pt-[7px] pb-[12px] pl-[18px] text-[#3D405A] font-Montserrat text-[14px] font-medium">Deposit or mint BGT-whitelisted LP tokens to earn iBGT (liquid BGT) & Boosted Yield.</div>
      ...
      <div className="flex items-center h-[90px] rounded-[10px] p-[18px] bg-[#FFDC50]">
        <div className='flex flex-col gap-[12px] w-[20%]'>
          ...
          <div className='text-black font-Montserrat text-[26px] font-semibold leading-[90%]'>{tvl}</div>
        </div>

        <div className='flex flex-col gap-[12px] w-[80%]'>
          <div className='text-[#3D405A] font-Montserrat text-[14px] font-medium'>APY up to</div>
          <div className='text-black font-Montserrat text-[26px] font-semibold leading-[90%]'>500%</div>
        </div>
      </div>
        ...
        {columnList.map((column: ColumnType, index: number) => {
          return (
            <div
              ...
              onClick={() => {
                column?.sort && updateState({
                  sortKey: state?.sortKey === column.key ? "" : column.key
                })
              }}
            >
              <div className='text-[#3D405A] font-Montserrat text-[14px] font-medium'>{column?.label}</div>
              {
                column?.sort && (
                  <svg xmlns="http://www.w3.org/2000/svg" width="13" height="8" viewBox="0 0 13 8" fill="none">
                    <path d="M5.37058 7.5C5.88774 8.16667 7.18062 8.16667 7.69778 7.5L12.3522 1.5C12.8693 0.833334 12.2229 4.76837e-07 11.1886 4.76837e-07H1.87979C0.845482 4.76837e-07 0.199039 0.833334 0.716193 1.5L5.37058 7.5Z" fill={state?.sortKey === column?.key ? "black" : '#D1CEB4'} />
                  </svg>
                )
              }
            </div>
        ...
        loading ? (
          <div className='flex items-center gap-[4px]'>
            {columnList.map((column: ColumnType) => {
              return (
                <Skeleton width={928 * parseInt(column?.width) / 100 - 4} height={58} />
              )
            })}
          </div>
            ...
            {state?.filterList.map((data: any, index: number) => {
              return (
                <div
                  key={index}
                  className={
                    clsx("flex items-center w-full h-[58px] rounded-[10px]", {
          ...
          <div className='flex justify-center'>You didn’t add any liquidity yet</div>
        )
      }
    </div>
  );
